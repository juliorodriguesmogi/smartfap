...
<link href="estilosistema.css" type="text/css" rel="stylesheet" />
       ...
       function popula_setores_historico_carregados(divisao,contador){
            ...
            function carregar_setores_agentes(){
                    ...
                    for(contador=0;contador<txtabela.length;contador++){
                        buscar_registros(window,txtabela[contador],'Carrega_Setores_Agentes_PPRA',campos,valores,true,Array("=","=","=","="),newpprassetores_agentes);
                        for (contador1=0;contador1<newpprassetores_agentes.length();contador1++){
                                ...
                                divpai.appendChild(divfilho);
                                buscar_registros(window,'estrutura_organizacional',"Carrega_Estrutura",Array("CODIGO_EMPRESA","UNIDADE"),Array(empresa_atual,newpprassetores_agentes.get_Unidade(contador)),false,Array('=','='),newEstruturas);      
                                campos_setores=Array();
                                valores_setores=Array();
                                campos_setores[0]='CODIGO_EMPRESA';
                                campos_setores[1]='CODIGO_SETOR';
                                valores_setores[0]=empresa_atual;
                                valores_setores[1]=newpprassetores_agentes.get_Codigo_setor(contador1);

                                buscar_registros(window,'setores','Carrega_Setores',campos_setores,valores_setores,true,Array("=","="),newSetores);
                                tbl=divpai.getElementsByTagName('table')[0];
                                ...
                                tbl.rows[2].cells[1].children[0].value=newpprassetores_agentes.get_Agente(contador);
                                linhaclone=tbl.rows[tbl.rows.length-1].cloneNode(true);
                                tbl.deleteRow(tbl.rows.length-1);
                                numocur=0;
                                quebra=newpprassetores_agentes.get_Agente(numcur);
                                do {
                                  linhaf=linhaclone.cloneNode(true);
                                  linhaf.rows[tbl.rows.length-1].cells[0].children[0].value=newpprassetores_agentes.get_Tipo(numocur);
                                  linhaf.rows[tbl.rows.length-1].cells[1].children[0].value=newpprassetores_agentes.get_Fonte_geradora(numocur);
                                  linhaf.rows[tbl.rows.length-1].cells[2].children[0].value=newpprassetores_agentes.get_Intensidade(numocur);
                                  linhaf.rows[tbl.rows.length-1].cells[3].children[0].value=newpprassetores_agentes.get_Grau_risco(numocur);
                                  tbl.appendChild(linhaf);
                                  quebra=newpprassetores_agentes.get_Agente(numcur);
                                  numocur++;
                                }	
                                while(numocur<newpprassetores_agentes.length() &&  newpprassetores_agentes.get_Agente(numcur)==quebra );
                        }
                    }
            }
...
<form>
<div id="div_setores_agentes" style="display:block;overflow:auto">
<table width="90%" border="0" class="tabela" >
<tr><td width="5%">Unidade </td>
  <td colspan="4"><input type="text" name="ppras_setores_agentes|UNIDADE[]" size="5" maxlength="5" />
    <input name="ppras_setores_agentes|DESCRICAO_UNIDADE[]" type="text" size="50" maxlength="50" /></td>
	
	</tr>
<tr><td>Setor </td><td colspan="4"><input name="ppras_setores_agentes|CODIGO_SETOR[]" type="text"  size="5" maxlength="5" />
  <input name="ppras_setores_agentes|NOME_SETOR[]" type="text" size="50" maxlength="50" /></td>
</tr>
...
<tr>
  ...
  <tr>
    <td><input name="ppras_setores_agentes_funcoes|DESCRICAO_FUNCAO[]" type="text" id="ppras_setores_agentes_funcoes|DESCRICAO_FUNCAO" size="50" maxlength="50" /></td>
...
<tr>
  <td><input type="text" name="ppras_setores_agentes|TIPO[]" size="5" maxlength="5" /></td>
  <td><textarea name="ppras_setores_agentes|FONTE_GERADORA[]" cols="40" wrap="physical"></textarea></td>
