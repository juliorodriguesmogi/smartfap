...
function entrada_numericaa(campo,evt){
               key=window.event?window.event.keyCode:evt.which; 
               var kcode=window.event?window.event.keyCode:evt.keyCode;
               var kcode=kcode.toString();
               var mascara=/^\d*$/; 
            ...
            function formata_decimal(campo,tipo){
             ...
             function formata_decimal_casas(campo,tipo,casas){
                numero_eu="###,###.###.###,##";
                numero_us="###,###,###,###.##";
                numero_real="###########";
                switch (tipo){
                ...
                parte_inteira=direita('00000000000000000'+parte_inteira,12);
                parte_decimal=parte_decimal+'0000';
                parte_decimal=parte_decimal.substr(0,casas);
                var campo_sem_mascara=parte_inteira+parte_decimal;
            ...
            function formata_hora(campo,tipo,evt){
                var kcode2=window.event?window.event.keyCode:evt.keyCode;
                var kcode2=kcode2.toString();
                var caracter_especial=" 9"+" 46"+" 8"+" 37"+" 38";   
                if (caracter_especial.indexOf(kcode2)>-1) return true;
    ...
    document.write("<link rel='stylesheet' href='800X600.css' type='text/css' />");
            ...
            function captura_valores(linha){
                if (navigator.appName.indexOf('Internet Explorer')>0){
                    celula=Array();
                    numero_colunas=linha.cells.length;
                    for(k=0;k<numero_colunas;k++){
                        celula[k]=linha.cells[k].innerText;
                    }    
                }
                else {
                    celula=Array();
                    numero_colunas=linha.cells.length;
                    for(k=0;k<numero_colunas;k++){
                        celula[k]=linha.cells[k].firstChild.textContent;
                    }    
...
function localizar_indice_componente(dvpai,nomecomp,tipocomp){
    contador=0;
    xt=true;
    componentes=dvpai.getElementsByTagName(tipocomp);
    for (xk=0;xk<componentes.length;xk++){
        if (componentes[xk].id==nomecomp){
            break;
        }
        else
            contador++;
