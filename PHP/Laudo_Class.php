...
class Laudo{
    ...
    function insere_laudo(){
        ...
        error_reporting(0);
        $this->oper=array("=","=","=","=","=","=","=","=","=","=","=");
        $conn->prepara_stmt('D', 'laudos_avaliacoes', null, null, $this->chave,$this->oper,$this->criterio);
        $r=$conn->executa_acao();
        $sqlq="show tables like '%laudos_%%_funcoes%';";
        $conn->prepara_stmt('C', null,null,null,null,null,null,null,null,null,$sqlq);
        $r=$conn->executa_acao();
        while ($row=$r->fetch(PDO::FETCH_BOTH)){
            $conn->prepara_stmt('D',$row[0],null,null,$this->chave,$this->oper,$this->criterio);
            
            $r1=$conn->executa_acao();
            
        }
        $sqlq="show tables like '%laudos_%%_maquinas%';";
        $conn->prepara_stmt('C', null,null,null,null,null,null,null,null,null,$sqlq);
        $r=$conn->executa_acao();
        while ($row=$r->fetch(PDO::FETCH_BOTH)){
            $conn->prepara_stmt('D',$row[0],null,null,$this->chave,$this->oper,$this->criterio);
                    ...
                    if ($campos[0]=='laudos_avaliacoes'){
                        foreach ($valores as $cmp=>$vlr){
                            ...
                            if ($contcomp>=4){
                                ...
                                $cy=0;
                                while ($i<count($vlr)){
                                    $v[$cy][$cmp]=is_array($vlr)?$vlr[$i]:$vlr;
                                    if ($v[$cy][$cmp]==1){
                                        $i++;
                                    }
                                    $i++;
                               ...
                               for ($i=0;$i<count($vlr);$i++){
                                  $v[$i][$cmp]=is_array($vlr)?$vlr[$i]:$vlr;
                                }
                            }
                        }
                    }
