...
class Laudo{
    ...
    function insere_laudo(){
        ...
        $r=$conn->executa_acao();        
        $conn->prepara_stmt('S', "laudos_epis_fornecidos", array('MAX(NUMERO_LAUDO) as NUMERO_LAUDO','MAX(DATA_CADASTRO) AS DATA_CADASTRO','MAX(ID_USUARIO) AS ID_USUARIO','MAX(ULTIMA_ATUALIZACAO) AS ULTIMA_ATUALIZACAO','MAX(ULTIMO_USUARIO) AS ULTIMO_USUARIO'), '');
    ...
    function apaga_laudo(){
        ...
        include_once '../classes/PHP/conexao_PDO.php';
        $conn=new conexao_PDO('209.97.130.137','smartfapdb','root','%phoenix17%','mysql');
        $conn->prepara_stmt('D', 'laudos_epis_fornecidos', $this->nome_campos, $this->valor_campos, array('NUMERO_LAUDO'), array('='),array($this->valor_campos[0],$this->valor_campos[1]));
