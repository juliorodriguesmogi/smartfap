...
<div align="right"><button type="button" onClick="window.print()"><img src='../images/printer_2.png'/></button></div>
   <table id="cabecalho_pagina" width="100%" style="border-bottom:solid 1px"  >
    ...
    <th colspan="2"><center>
      <h2>Relatório de Fabricantes cadastrados </h2>
    </center></th>
	<th></th>		
  </tr>
  <tr>
...
<table width="100%" >    
  <tr id="cabecalho_campos" style="border-bottom:solid 1px" >
    ...
    <td width="25%" rowspan="2" valign="top">Nome </td>
    <td  width="12%" rowspan="2" valign="top">Cnpj </td>
    <td  width="21%" rowspan="2" valign="top">Descrição </td>
    <td  width="17%" rowspan="2" valign="top">Data de Cadastro </td>
    <td  width="19%" rowspan="2" valign="top">Última Atualização </td>
  </tr>
  <tr>
  </tr>
<?php
...
$conn->prepara_stmt('S', 'fabricantes',array('*'), null,null , null,null,array("CODIGO_FABRICANTE"));
...
?>
    ...
    <td width="6%"><?php echo $row['CODIGO_FABRICANTE']?></td>
    <td width="25%"><?php echo mb_strtoupper($row['NOME'])?></td>
      <td width="12%"><?php echo $row['CNPJ']?></td>
        <td width="21%"><?php echo $row['DESCRICAO']?></td>
   
	<td width="17%" style="font-size:10px"><?php echo preg_replace("/(\d+)-(\d+)-(\d+) (\d+):(\d+):(\d+)/", "$3/$2/$1 $4:$5:$6", $row['DATA_CADASTRO'])?></td>
    <td width="19%" style="font-size:10px;max-length:100px;" align="left" ><?php echo preg_replace("/(\d+)-(\d+)-(\d+) (\d+):(\d+):(\d+)/", "$3/$2/$1 $4:$5:$6", $row['ULTIMA_ATUALIZACAO'])?> </td>
  </tr>
  <?php 
  }
}
