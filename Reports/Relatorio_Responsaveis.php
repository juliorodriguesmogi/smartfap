...
<div align="right"><button type="button" onClick="window.print()"><img src='../images/printer_2.png'/></button></div>
   <table id="cabecalho_pagina" width="100%" style="border-bottom:solid 1px"  >
    ...
    <th colspan="2"><center>
      <h2>Relatório de responsaveis cadastrados </h2>
    </center></th>
	<th></th>		
  </tr>
  <tr>
...
<table width="100%" >    
  <tr id="cabecalho_campos" style="border-bottom:solid 1px" >
    <td  width="6%" valign="top">Codigo</td>
    <td  width="6%" valign="top">Nome </td>
   <td  width="4%" valign="top">RG </td>
   <td  width="6%" valign="top">Data de admissao </td>
   <td  width="3%" valign="top">NIT </td>
...
$conn->prepara_stmt('S', 'responsaveis',array('*'), null,null , null,null,array("CODIGO_RESPONSAVEL"));
...
?>
    ...
    <td width="6%"><?php echo $row['CODIGO_RESPONSAVEL']?></td>
    <td width="13%"><?php echo mb_strtoupper($row['NOME_RESPONSAVEL'])?></td>
   <td width="13%"><?php echo mb_strtoupper($row['RG'])?></td>
   
   <td width="13%"><?php echo preg_replace("/(\d+)-(\d+)-(\d+)/", "$3/$2/$1", $row['DATA_ADMISSAO'])?></td>
   <td width="13%"><?php echo mb_strtoupper($row['NUMERO_NIT'])?></td>
